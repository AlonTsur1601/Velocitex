...
$output = & $godot.FullName --headless --fixed-fps 60 --path $root "res://scenes/BlenderRoomExactSmokeTest.tscn" --quit-after 900 -- --blender-room-exact-smoke 2>&1
...
if (($output -join "`n") -match "ERROR:|ObjectDB instances were leaked|resources still in use") {
    throw "Blender exact smoke reported an error or leaked resource."
}
